...
import com.app.controller.LearnerLicenseApplicationController;
...
public class AdminServiceImpl implements AdminService {
	...
	public List<UserDTO> getAllUsersPaginated(int pageNumber) {
		...
		log.info("got the users list , inside admin controller");
...
//		.filter(owner->!owner.getVehicles().isEmpty())
	...
	@Override
	public List<Object> getVehicleService(String regNo) {
		
		Vehicle vehicle=vehicleDao.getByRegistrationNumber(regNo);
		if(vehicle!=null)
		{
			OwnerDTO vehicleOwner= mapper.map(vehicle.getOwner(),OwnerDTO.class);
			log.info(vehicleOwner.toString());
